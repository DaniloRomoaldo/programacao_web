...
export const visulaizarAluno = async (req, res) => {
    ...
    try {
        const aluno = await alunoService.visulaizarAluno(parseInt(req.params.id));
...
export const atualizarAlunoParcial = async (req, res) => {
    ...
    try {
        await alunoService.atualizarAlunoParcial(parseInt(req.params.id), req.body);
        res.status(204).send();
    } catch (error) {
        res.status(400).json({"message": "Erro ao processar a requisição."});
    }
...
export const deleteAluno = async (req, res) => {
    ...
    try {
        await alunoService.deleteAluno(parseInt(req.params.id));
        res.status(204).send();
    } catch (error) {
